...
let partial_sub (a: t) (b: t) =
  let width = Signal.width a in
  let res = mux2 (a <+ of_signed_int ~width 0) (a +: b) (a -: b) in
  mux2 ((a <+ b) &: (a >+ of_signed_int ~width 0)) a res
;;
...
let create_circuit a_width b_width = 
  ...
  Circuit.create_exn ~name:"div_mod" [ output "modulo" modulo; output "divisor" divisor; output "valid" valid ];;