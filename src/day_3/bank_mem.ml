...
let bank_mem (_scope : Scope.t) (inputs : Signal.t I.t) =
  let size = 12 in
  ...
  let mem =
    Array.init size
    @@ fun idx ->
    Signal.reg spec update_lines.(idx) ~enable:(select inputs.write_mask idx idx)
  in
  Signal.(update_lines.(0) <== inputs.write_lsp);
...
let hierarchical (scope : Scope.t) instance (input : Signal.t I.t) =
  ...
  H.hierarchical ~scope ~name:"bank_mem" ~instance bank_mem input
;;
